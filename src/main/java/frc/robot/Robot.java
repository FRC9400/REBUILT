...
public class Robot extends LoggedRobot {
  ...
  public void robotInit() {
    ...
    /*RobotModeTriggers.autonomous().onFalse(
      Commands.runOnce(() -> m_robotContainer.getSwerve().requestDesiredState(0,0,0,true,false))
    );*/
  }
  ...
  public void autonomousInit() {
    ...
    boolean seeded = m_robotContainer.getSwerve().seedGyroOnlyFromLimelight();

    if (!seeded) {
    m_robotContainer.getAutos().getSelectedStartingPose().ifPresentOrElse(
        pose -> {
            m_robotContainer.getSwerve().setGyroStartingPosition(pose.getRotation().getDegrees());
            DriverStation.reportWarning("[Auto] Vision failed: seeded gyro from auto starting pose", false);
        },
        () -> {
            DriverStation.reportWarning("[Auto] Vision failed: zeroing gyro", false);
